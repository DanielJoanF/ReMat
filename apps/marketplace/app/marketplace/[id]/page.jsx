...
function QrisPaymentModal({ totalAmount, material, orderId, distributorPhone, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm max-h-[95vh] overflow-y-auto animate-slide-up flex flex-col">
        {/* Header */}
        <div className="p-4 pb-2 border-b border-gray-100">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <QrCode className="w-4 h-4 text-remat-green" />
              <h3 className="font-bold text-gray-900 text-sm">Bayar via QRIS</h3>
            </div>
            <button onClick={onClose} className="p-1 rounded-lg hover:bg-gray-100 transition-colors">
              <X className="w-4 h-4 text-gray-500" />
            </button>
          </div>
          <p className="text-[10px] text-gray-500 mt-0.5">Scan QR code di bawah menggunakan aplikasi e-wallet atau mobile banking Anda</p>
        </div>

        {/* QRIS Code */}
        <div className="px-4 py-2 flex flex-col items-center">
          <div className="p-2 border-2 border-dashed border-remat-green/40 rounded-xl bg-gray-50">
          ...
          {/* Amount */}
          <div className="mt-2.5 w-full p-2.5 bg-remat-green-light rounded-xl flex items-center justify-between">
            <div>
              <p className="text-[10px] text-gray-500">Nominal Pembayaran</p>
              <p className="font-black text-remat-green text-base">{formatIDR(totalAmount)}</p>
            </div>
            <button
              onClick={handleCopy}
              className="text-[11px] font-medium text-remat-green border border-remat-green/30 px-2.5 py-1 rounded-lg hover:bg-remat-green/10 transition-colors"
            >
          ...
          <p className="text-[10px] text-gray-400 text-center mt-2">
            <span className="font-semibold text-gray-600">Silahkan melakukan konfirmasi pembayaran ke nomor WhatsApp di bawah</span>.             
          </p>
        </div>
        ...
        <div className="p-4 pt-1">          
          <a
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-[#25D366] hover:bg-[#1ebe5b] text-white font-semibold rounded-xl transition-colors text-xs"
          >
            {/* WhatsApp icon SVG */}
          ...
          <Link
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2 mt-2 text-xs font-medium text-gray-600 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors"
            onClick={onClose}
          >
            Lihat Pesanan Saya
          </Link>
